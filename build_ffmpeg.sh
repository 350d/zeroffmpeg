...
# Compute CFLAGS (no --static)
CFLAGS="$ARCH_FLAGS $(pkg-config --cflags libv4l2 libv4lconvert openssl libdrm x264 zlib libmp3lame opus ogg vorbis libjpeg)"
echo "CFLAGS=$CFLAGS"

# Compute LDFLAGS using --static for libs
LDFLAGS="$(pkg-config --static --libs libv4l2 libv4lconvert openssl libdrm x264 zlib libmp3lame opus ogg vorbis libjpeg) -static"
